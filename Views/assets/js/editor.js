    ...
    $.fn.richText = function(options) {
        ...
        function init() {
            ...
            if ($inputElement.prop('tagName') === 'TEXTAREA') {
                ...
                $inputElement.replaceWith(
                    $('<textarea' + attributes_html + ' data-richtext="init">' + value + '</textarea>')
                );
        ...
        settings.$editor.find('.richText-help').on('click', function() {
            ...
            if ($editor) {
                ...
                var $content = $('<div />', {
                    html: '<span id="closeHelp" style="display:block;position:absolute;top:0;right:0;padding:10px;cursor:pointer;" title="' +
        ...
        settings.$editor.find('.richText-toolbar a[data-command]').on('click', function(event) {
            ...
            if (
                $editor.length > 0 &&
                id === editorID &&
                (!$button.parent('li').attr('data-disable') || $button.parent('li').attr('data-disable') === 'false')
            ) {
                event.preventDefault();
                ...
                if (command === 'toggleCode') {
                    ...
                    $editor
                        .find('div:empty,p:empty,li:empty,h1:empty,h2:empty,h3:empty,h4:empty,h5:empty,h6:empty')
                        .remove();
       ...
       */
                    ...
                    if (!e.shiftKey) {
                        ...
                        textNode =
                            ...
                            sel.focusNode.parentNode.previousSibling.childNodes[
                                sel.focusNode.parentNode.previousSibling.childNodes.length - 1
                            ] :
                            null :
                            sel.focusNode.parentNode.previousSibling != null ?
                            sel.focusNode.parentNode.previousSibling :
                            sel.focusNode.parentNode.parentNode.previousSibling != null ?
                            sel.focusNode.parentNode.parentNode.previousSibling.childNodes[
                                sel.focusNode.parentNode.parentNode.previousSibling.childNodes.length - 1
                            ] :
                            null;
                    }
        ...
        function updateMaxLength(editorID) {
            ...
            $editor
                ...
                .html('<span class="' + color + '">' + content.length + '</span>/' + maxLength);
        ...
        function getVideoCode(url, size) {
            ...
            if (video.platform === 'YouTube') {
                ...
                html +=
                    ...
                    size[1] +
                    '" frameborder="0"' +
                    (responsive === true ? ' style="position:absolute;width:100%;height:100%;left:0"' : '') +
                    ' ' +
                    allowfullscreen +
                    '></iframe>';
                success = true;
